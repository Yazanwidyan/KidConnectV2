...
const TeacherSidebar = () => {
  ...
  // Filter only teacher menu items
  const menuItems = SIDEBAR_MENU.filter((menu) => menu.roles.includes("TEACHER"));

  return (
    <div className={`h-screen bg-white shadow-lg transition-all duration-300 ${collapsed ? "w-16" : "w-64"}`}>
      <div className="flex items-center justify-between border-b px-4 py-3">
        <span className={`text-lg font-bold ${collapsed ? "hidden" : "block"}`}>Teacher Panel</span>
