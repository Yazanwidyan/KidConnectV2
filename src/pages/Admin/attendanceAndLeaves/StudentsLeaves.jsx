...
const StudentsLeaves = () => {
  ...
  const filteredRequests = useMemo(() => {
    let data = requests.filter((r) => {
      return (
        (filters.student === "" || r.student.toLowerCase().includes(filters.student.toLowerCase())) &&
  ...
  const resetFilters = () => setFilters({ student: "", type: "", status: "", startDate: "", endDate: "" });
  ...
  const saveNewLeave = () => {
    ...
    setNewLeave({ student: "", type: "", startDate: "", endDate: "", comment: "", attachment: null });
    setAddModal(false);
  };
    ...
    <div className="w-full p-6">
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between">
        <div aria-label="Breadcrumb">
          <h1 className="text-2xl font-bold text-black">Student Leave Requests</h1>
          <nav className="flex" aria-label="Breadcrumb">
            <ol className="inline-flex items-center space-x-2">
              <li className="inline-flex items-center">
                <div className="flex items-center text-sm font-semibold text-black">
                  <UserCircleIcon className="h-4 w-4 stroke-2" /> <h5>Students</h5>
                </div>
              </li>
              <span className="text-xs text-gray-500">/</span>
              <li aria-current="page">
                <span className="text-sm font-semibold text-primary">Student Leave Requests</span>
              </li>
            </ol>
          </nav>
        </div>
      ...
      <div className="overflow-x-auto rounded-lg bg-white shadow-lg">
        <table className="min-w-full divide-y divide-dashed divide-gray-400/60">
          <thead className="bg-gray-50">
            <tr>
              {["Student", "Type", "Start Date", "End Date", "Status", "Actions"].map((col) => (
                <th
                  key={col}
            ...
            {paginatedRequests.length > 0 ? (
              paginatedRequests.map((req) => (
                <tr key={req.id} className="transition odd:bg-gray-100 even:bg-white hover:bg-gray-50">
                  <td className="px-6 py-3">{req.student}</td>
      ...
      {addModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-lg">
            ...
            {["student", "type", "startDate", "endDate"].map((field) => (
              <input
                key={field}
      ...
      {viewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-lg">
            ...
            {["student", "type", "startDate", "endDate"].map((f) => (
